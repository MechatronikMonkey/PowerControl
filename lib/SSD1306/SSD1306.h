...
public:
	 ...
	 */
	SSD1306_Spi(SPI &spi, PinName DC, PinName RST, PinName CS, uint8_t rawHieght = 32, uint8_t rawWidth = 128)
	    : SSD1306(RST, rawHieght, rawWidth)
	    , cs(CS,true)
	    , dc(DC,false)
	    , mspi(spi)
	    {
...
public:
	 ...
	 * @param rawWidth - The horizonal number of pixels for the display, defaults to 128
	 */
	SSD1306_I2c(I2C &i2c, PinName RST, uint8_t i2cAddress = SSD_I2C_ADDRESS, uint8_t rawHeight = 32, uint8_t rawWidth = 128)
	    : SSD1306(RST, rawHeight, rawWidth)
	    , mi2c(i2c)
